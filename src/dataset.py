...
from src.preprocessing import load_gff, load_similarity_score, load_ribap_groups, build_edge_index, map_edge_weights, map_labels_to_edge_index, construct_neighbour_lst, generate_neighbour_edge_features, build_adjacency_vectors, normalize_sim_scores, remove_trivial_cases
    ...
    def __init__(self, gff_files = [], similarity_score_file = '', ribap_groups_file = None, split = (0.7, 0.15, 0.15), categorical_nodes = False, calculate_baseline = False):
        ...
        if not gff_files or args.simulate_dataset:
            if not args.simulate_dataset:
                ...
                if not args.include_trivial: self.sim_score_dict_raw = remove_trivial_cases(self.sim_score_dict_raw)
