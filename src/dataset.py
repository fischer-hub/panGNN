from src.preprocessing import load_gff, load_similarity_score, load_ribap_groups, build_edge_index, map_edge_weights, map_labels_to_edge_index, construct_neighbour_lst
from src.setup import log
import torch, os

class HomogenousDataset():
    def __init__(self, gff_files = [], similarity_score_file = '', ribap_groups_file = None, num_neighbours = 1):
        ...
        self.gene_id_integer_dict = {gene: idx for idx, gene in enumerate(gene_ids_lst)}
        ...
        self.edge_weight_ts = map_edge_weights(self.edge_index_ts, sim_score_dict, gene_ids_lst)#torch.randn((num_genes/2, edge_feature_dim))  # Edge 
        ...
        if ribap_groups_file:
            ...
            self.labels_ts = map_labels_to_edge_index(self.edge_index_ts, gene_ids_lst, self.ribap_groups_dict)
